...
//@copy this annotation should be kept as it is
    ...
    // Long comment with empty lines Long comment with empty lines Long comment with empty lines Long comment with empty lines
    //
    //
    //
    //
    struct structLongCommentWithEmptyLines {
        double x;
    };
    ...
    // need to be escaped " \
    // also in the middle " \ of the sentence
    // these are not escaped ( ) ,
    module TestModuleName1
    {
        enum AnotherEnum
        {
            Element0, // escape characters " \ in the comment
            Element1, // escape characters in the comment at the end " \
            Element2, // This is an inline long comment with comma - long comment long comment long comment long comment long comment long comment long comment long comment long comment long comment long comment long comment
            Element3 //This is an inline long comment without comma - long comment long comment long comment long comment long comment long comment long comment long comment long comment long comment long comment long comment
        };
    };
    ...
    //
    // and an empty line in the middle and at the end
...
def add_doc_annotation_from_comment(
        ...
        fixed_indentation=-1):
    if comment_buffer:
        # if any element of comment_buffer list only contains whitespaces, ignore it
        comment_buffer_result = []
        for line in comment_buffer:
            # remove tailing backslash since codegen doesn't support it
            if line.endswith('\\'):
                line = line[:-1]
            if line.strip() != '':
                comment_buffer_result.append(line)
        comment_buffer[:] = comment_buffer_result
        # join all lines in comment_buffer with '\n' to indicate multiline comment
        comment = '\n'.join(comment_buffer)
...
def escape_characters(input_string):
    ...
    # The first character to escape is the backslash, to avoid double escaping
...
def parse_idl_string_to_doc(idl_string, debug=False):
    """Parse IDL strings to translate comments to @doc annotations."""
    # create temporal file with the content of idl_string
    temp_file = 'temp_idl_file.idl'
    with open(temp_file, 'w') as file:
        file.write(idl_string)
    result = parse_idl_file_to_doc(temp_file, debug)
    os.remove(temp_file)
    return result

def parse_idl_file_to_doc(idl_file, debug=False):
...
def process_files(input_folder, output_folder):
    """
        Processes .idl files in input folder, applies transformations, and saves
        them in output folder.
    """
    # process all files
        ...
        for file_name in files:
            ...
            if file_name.endswith('.idl'):
                with open(output_file, 'w') as file:
                    result_lines = parse_idl_file_to_doc(input_file)
